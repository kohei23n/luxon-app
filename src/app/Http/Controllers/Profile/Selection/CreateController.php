...
namespace App\Http\Controllers\Profile\Selection;
...
class CreateController extends Controller
{
    public function add()
    {
        return view('profile.selection.add');
    }
    ...
    public function create(Request $request): RedirectResponse
    {
        ...
        if ($status) {
            return Redirect::route('profile.index')->with('status', 'selection-status-created');
        } else {
            return Redirect::route('profile.index')->with('error', 'error-creating-selection-status');
        }
    }
}
